...
float lepton_permut(int pdgid1, int pdgid2 ,int  pdgid3)
{
  cout << " kfghdks" <<endl; 
...
float triggerSF_3l(float _pt1, float _eta1, float _pt2, float _eta2, float _pt3, float _eta3, float _met, float _met_corr, float choose_leptons){
  ...
  if(_met>=200.0 && _met_corr>=200.0)
  {
   ...
   ///// Low MET
  if((_met>=75.0 && _met_corr>=75.0) &&( _met<125.0 && _met_corr<125.0)){
  if (!_file_triggerSF) init_triggerSF_SOS();{
    float eta1 = std::min(float(2.399),abs(_eta1));
    float eta2 = std::min(float(2.399),abs(_eta2));
...
//  if(var<0) return res-0.05; // -5%
  return res;
  cout<< " MET = "<< _met<< "res = " << res<< endl;
  }
}
  ///// Intermediate MET
  if (_met>=125.0 && _met_corr>=125.0 && _met<200.0 && _met_corr<200){
    ...
    if (choose_leptons ==123){
      ...
      float met_mc = 1.0;
      float dca_data= 0.906; // DCA SF (inclusive) 
      cout << "Inter BIN return is : " << (mu_data*dca_data*met_data)/(mu_mc*met_mc) << endl;   
...
//  if(var<0) return res-0.05; // -5%
  ...
  ///// Intermediate MET
  if(_met>=125.0 && _met_corr>=125.0  && _met<200.0 && _met_corr<200.0){
    if (choose_leptons==12){
